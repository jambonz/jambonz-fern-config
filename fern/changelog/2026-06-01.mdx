...
1. **Groq LLM support** — Add Groq as an LLM provider, exposing Groq's low-latency inference of open-weight models (Llama, Mixtral, and others) to the agent verb and `llm.toolHook` flows.
1. **HuggingFace Inference Providers** — Add HuggingFace Inference Providers as an LLM provider, opening up the broad catalog of models served through the HuggingFace inference network.
1. **Baseten LLM support** — Add Baseten as an LLM provider, letting you wire Baseten-hosted open-weight model deployments directly into the agent verb.
