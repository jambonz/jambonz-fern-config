...
1. **OpenAI Realtime GA** — Full support for OpenAI's general-availability Realtime API. The platform detects the session format on the wire and converts legacy formats transparently while stripping GA-invalid fields from older `response_create` payloads.
