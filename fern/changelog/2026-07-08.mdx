...
1. **Bidirectional conference listen stream** — The conference listen fork is **bidirectional**: the room's mixed audio streams to your WebSocket endpoint, and audio the WebSocket server streams back is mixed into the room and heard by every participant (unless disabled with `disableBidirectionalAudio`).
1. **Play or speak to a whole room** — The media server can play an audio file (or tone) and speak TTS to an entire conference/room, mixed into the room mix so all participants hear it, with the ability to stop an in-flight playout.
