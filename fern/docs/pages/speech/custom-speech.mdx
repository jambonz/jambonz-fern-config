...
jambonz natively supports a large number of [Speech to Text (STT)](/verbs/verbs/recognizer) and [Text to Speech (TTS)](/verbs/verbs/say) speech providers out of the box, but if you want to use a speech provider that we don't support: no propblem! You can also add support for new speech vendors using our Speech APIs:

- [custom STT API](/guides/speech-providers/custom-speech-stt) (websocket-based)
- [custom TTS API](/guides/speech-providers/custom-speech-tts) (http-based)
...
![Creating custom STT vendor](../../assets/images/creating-custom-stt-vendor.png)

Then, in the Application view simply select your custom vendor:

![Selecting custom speech vendor](../../assets/images/using-custom-speech.png)
