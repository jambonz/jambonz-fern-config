...
## Background
Application Environment Variables (app-env vars) are a feature in Jambonz that allows you to pass custom values to your applicaiton in the call webhook. 
...
The values for the parameters are then entered during application setup and stored in encrypted in the database.

When a call is routed to the applicaiton these parameters are sent in the calling webhook.
...
If the applicaiton does not respond with a valid JSON schema or does not handle the OPTIONS request then Jambonz will silently ignore the error and the application configuration can continue as normal.
...
If set to True then the parameter must be defined in order for the applicaiton to be created
</ParamField>
...
- `textarea`
- `filepicker`
When type is set to string the defualt is a simple one line text box, `textarea` allows for a larger multiline input 
...
App Env Vars may also be created, read and modified on an applicaiton via the [REST API](/reference/rest-platform-management/applications/create-application#request.body.env_vars)
