...
For Google's own Gemini models, use [Vertex AI — Gemini](/guides/features/llm-vendors/vertex-gemini) instead.

## Get credentials

Identical to Vertex Gemini — you need a service account JSON. See the [Vertex AI — Gemini](/guides/features/llm-vendors/vertex-gemini#get-credentials) page for full steps.
...
<ParamField path="Region" type="select" required={true}>
  Pick a region where the partner model you intend to use is hosted. **Llama models are most broadly available in `us-east5`.** Other partner models may have different region availability — check the [Vertex partner-model docs](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-openai) before picking.
</ParamField>
