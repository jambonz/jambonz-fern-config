...
This adapter is **not** for HuggingFace's older shared Inference API (different wire shape) or dedicated Inference Endpoints (those are per-customer URLs that work with `vendor: openai` and a `baseURL` override).
