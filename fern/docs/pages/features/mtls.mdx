...
2. A certificate must carry the `clientAuth` extended key usage to be usable as a client
   ...
   `unsuitable certificate purpose` no matter how well it is trusted.
...
| `unsuitable certificate purpose` | your certificate is missing the `clientAuth` extended key usage |
