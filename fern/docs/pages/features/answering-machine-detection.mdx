...
description: Configure answering machine detection in jambonz to identify voicemail and human answers on outbound calls. Tuning parameters explained.
hidden: false
---
...
<ParamField path="digitCount" type="number" required={false} default="0">
  Number of digits in a greeting to trigger a `amd_machine_detected` result. 0 is off 
</ParamField>
...
### Digit Count
Many voicemail systems (especially on cellular) include the called number as part of their default greeting, the transcription returns these digits as a single "word" eg "07790123900 is not availble",
therefore the `thresholdWordCount` is often not reached for what is in effect a long message.<br/>
By default the feature is off (digitCount = 0) so won't trigger. The length is adjustable for different scenarios and countries, with UK numbers I've found that 5-6 is a good value.<br/>
There are some humans (in the UK at least) that still answer their landlines with the phone number eg "London 212555" so you may need to consider your audience with this feature.

### Beep detection
The feature also attempts to detect audio tones, or beeps that are commonly used on voicemail systems.  If detected, an event is sent via the actionHook to the webapp.
