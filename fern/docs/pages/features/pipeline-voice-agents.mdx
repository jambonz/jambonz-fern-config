---
title: Pipeline Voice Agents
subtitle: Build conversational AI agents using the pipeline verb
---
...
The [pipeline](/verbs/verbs/pipeline) verb orchestrates a complete voice AI agent by wiring together three separate components — STT, LLM, and TTS — with integrated turn detection. Unlike the [llm](/verbs/verbs/llm) verb (which connects to speech-to-speech APIs where a single vendor handles everything), the pipeline verb lets you mix and match: for example, Deepgram for STT, Anthropic for the LLM, and Cartesia for TTS.
...
Here is a minimal pipeline agent using the [Node.js SDK](/sdks/node-sdk):
...
Or as JSON (for webhook applications):
...
The pipeline normalizes message formats and tool schemas across vendors automatically. You write tools in OpenAI format and the pipeline adapts them for each vendor.

### Authentication

By default, the pipeline uses speech credentials configured in the jambonz portal. You can also pass credentials directly:
...
Set to `0` to disable (default). The timer is cancelled if the user starts speaking.
...
The pipeline supports LLM tool/function calling, allowing your agent to perform actions like looking up data, calling APIs, or transferring calls.
...
The pipeline normalizes tool schemas across LLM vendors. You always define tools in the same format regardless of whether you're using OpenAI, Anthropic, Google, or Bedrock.
...
Instead of (or in addition to) defining tools inline, you can connect to external [MCP](https://modelcontextprotocol.io/) servers. The pipeline connects to each server at startup via SSE or Streamable HTTP transport, discovers available tools, and makes them callable by the LLM.
...
The pipeline supports asynchronous updates while a conversation is in progress, allowing you to change the agent's behavior, inject context, modify tools, or trigger responses — without interrupting the verb stack.

Updates are sent via WebSocket (`session.updatePipeline(data)`) or REST API.
...
Prompt the LLM to generate a new response. If the pipeline is idle, the prompt executes immediately. If busy, the request is queued.
...
For long conversations that might exceed the LLM's context window, the pipeline can automatically summarize older turns.
...
The pipeline handles errors gracefully to keep the conversation going:

- **LLM errors with tools** — If the LLM fails and tools were included, the pipeline retries the same prompt without tools. This handles models that don't support tool use in certain configurations.
...
- **Recovery to idle** — On unrecoverable LLM errors, the pipeline ends the turn and transitions to idle so the user can continue speaking. The no-response timer is not started after an error to avoid retry loops.
- **STT reconnection** — The pipeline automatically reconnects the STT stream if the connection drops.

When the pipeline encounters an unrecoverable error, it invokes the `actionHook` with a `completion_reason` indicating the failure.

## Example Applications

The [pipeline examples](https://github.com/jambonz/v10-examples/tree/main/examples/pipeline) repository contains runnable demos for each feature:

| Example | What it demonstrates |
|---------|---------------------|
| `deepgram-cartesia` | Basic pipeline with Deepgram STT + Cartesia TTS |
