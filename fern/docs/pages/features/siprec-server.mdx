---
title: Agent Assist via SIPREC
description: Configure a SIPREC server on jambonz for agent assist and call recording. Learn SIPREC session setup, media handling, and requirements.
hidden: false
---
...
In this scenario, jambonz acts as a SIPREC server and receives incoming audio streams signaled in the SIPREC INVITE from the SBC.  These audio streams can then be handled by a jambonz application, which can transcribe the audio as well as send it to 3rd parties via the [listen](/verbs/verbs/listen) verb.
...
- [config](/verbs/verbs/config)
- [gather](/verbs/verbs/gather)
- [transcribe](/verbs/verbs/transcribe)
- [listen](/verbs/verbs/listen)
