...
For systems already deployed, add the Krisp API key to the mediajam environment file on the feature-server or mini. As of jambonz 11, mediajam is the media server and reads its configuration from `/etc/default/mediajam` — the key no longer belongs in the FreeSWITCH systemd unit.

1. Edit the mediajam environment file:
   ```bash
   sudo vi /etc/default/mediajam
   ```

2. Set the `KRISP_API_KEY` variable to your API key (the file ships with an empty value):
   ```ini
   KRISP_API_KEY=your-api-key-here
   ```
...
<Note>
Krisp is initialized only when `KRISP_API_KEY` is non-empty at startup, and licensing is validated against `sdkapi.krisp.ai`, so the media server needs outbound HTTPS to that host.
</Note>
