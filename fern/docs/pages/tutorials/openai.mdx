...
Apps written for the preview API still work without changes — jambonz auto-converts the legacy flat shape (`voice`, `input_audio_format`, `input_audio_transcription`, `turn_detection` directly under `session_update`) to the GA shape on the wire, logs a one-time deprecation warning, and aliases GA event names back to the preview names so your existing `events` filters keep matching. Plan to update to the GA shape before that compatibility layer is removed.

See the [`llm` verb article](/verbs/llm) for the canonical GA-shape example and field reference.
</Note>
