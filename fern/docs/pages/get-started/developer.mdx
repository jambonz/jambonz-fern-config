...
description: Get started with jambonz in minutes. Install, connect a carrier, create your first application, and place a test call.
---
...
jambonz supports both a webhook and a websocket API for creating user applications, as well as a choice of three SDKs you can use. These will be described below.
...
in building asynchronous applications, which you are likely to need eventually. Also, some advanced features require the websocket API.  
So we'll look at this first.

## Building a jambonz application using websockets

Let's use our npx application generator to scaffold a websocket application.
...
- echo: a collect-and-response app that echos caller voice input
...
This tool will let us scaffold out a new application using one of several scenarios.

Let's start with a basic "echo tester" app that performs speech-to-text on the caller's utterances and 
repeats it back using text-to-speech. This is a useful little app to test out a new speech 
engine or verify that a new jambonz system you just provisioned is working properly.
...
Now we have a websocket application listening for incoming connections from jambonz on port 3000. 
You can see that in this case, I am running it on my laptop, but in a production environment, you would typically run this on a
...
Let's look at the code before configuring our jambonz server to connect to this application.

The main app.js file is boilerplate code that you will rarely need to change, but let's take a quick look at it:
...
The handlers for various URL paths can be found in the lib/routes folder. This is where you can find the 
actual application logic. Let's take a look at the echo.js file, which implements 
...
the default speech recognizer and text-to-speech engines, but we could override those choices here if we wanted to change
the speech vendor for a single turn of the conversation.
...
Now that our application is running, we need to configure jambonz to use it. This is done in the [jambonz portal](https://jambonz.cloud/)
by creating a new application.  
- Click on Applications in the left-hand navigation, then click the "+" button.
- Give your application a name, then put your websocket URL in both the `Calling webhook` and `Call status webhook` fields. Your websocket URL needs to start with "wss://"
- Select speech vendors for both speech synthesis and speech recognition, then choose voice and language settings.
...
Your application will be triggered when you call that number, and you can test it out!
...
Now that we've seen how to build a websocket application, let's look at creating a webhook application.
...
The routes are defined in the lib/routes/endpoints folder. Let's look at the code for "hello-world":
...
Note the similarities to the websocket application: in both cases, we have a `Session` object with methods 
corresponding to the jambonz verbs. Running this application is exactly the same as running the websocket application, except that you need to provide 
the additional environment variables.  

Provisioning the application in jambonz is also similar. 
The only difference is that the call status webhook differs from the calling webhook. This is because
...
This guide shows you how to install the jambonz package for Node-RED and set up ngrok to allow jambonz to send webhooks to Node-RED running on your local machine.
...
- Set up your [jambonz account](https://jambonz.cloud/) with a carrier, phone number and speech provider.
...
9\. On the left-hand side is the palette, scroll down to find the ngrok node.
...
15\. Scroll the palette to the bottom, and you will see the jambonz nodes. Drag a "webhook in" node to the canvas, then double-click it to open the config.
...
21\. Drag a webhook out node onto your canvas.

![](../../assets/images/nodered/2c6b1c8c-32bb-4b92-aaa1-1ddf3a61b79d.jpeg)


22\. Wire the nodes together so that the webhook In is connected to say and then say to webhook out.

![](../../assets/images/nodered/0706cec4-e81b-42e4-87b6-368ffd8737bf.jpeg)


23\. Drag a debug node from the palette to the canvas and wire it to the ngrok node.

![](../../assets/images/nodered/af5fdc49-8ab5-4a46-9a7f-e483bae64c7a.jpeg)


24\. Now click "Deploy" to save your flow.

![](../../assets/images/nodered/dd827242-1a56-437d-9ad5-0c4a54d089b4.jpeg)


25\. Click the button on the ngrok node to activate your ngrok tunnel.

![](../../assets/images/nodered/d9c88430-60bd-4dae-8ef0-f2c90b60f668.jpeg)


26\. Now click on the debug icon in the right-hand panel.

![](../../assets/images/nodered/b34e194b-3c3d-44bb-95b1-639e62736d73.jpeg)


27\. You should see the ngrok hostname for your session in the debug bar, click the icon to copy that value to your clipboard.

![](../../assets/images/nodered/f9ff4933-2550-4472-baac-b3b2d3bf86f9.jpeg)


28\. In a new tab, navigate to [https://jambonz.cloud/](https://jambonz.cloud/)
...
44\. If you now call your number, you will hear the text configured in the say node.
