...
description: Add a SIP carrier in jambonz and configure trunk settings, authentication, and gateways for inbound and outbound voice traffic.
---
...
give them to authenticate with.  

<Frame caption="SIP Digest credentials for Auth trunk">
  <img src="../../assets/images/auth-trunk-creds.jpg" alt="Auth trunk credentials"/>
</Frame>

You will also tell them the SIP realm associated with your jambonz account, as they 
will need this as well.  You can find your SIP realm in the jambonz portal on the Account page.

<Frame caption="SIP realm for an account">
  <img src="../../assets/images/sip-realm.jpg" alt="Finding sip realm value"/>
</Frame>
...
resolve to a set of IP addresses, if the Outbound Gateway has a port number set then we will do an A record lookup for the IPs 
if there is no port number defined then  we will do a SIP SRV lookup.
Any INVITEs received from those IPs will automatically be accepted as coming from the remote side of the SIP trunk.
