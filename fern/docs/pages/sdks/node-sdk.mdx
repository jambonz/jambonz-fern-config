---
title: Node.js SDK
subtitle: Build jambonz voice applications with @jambonz/sdk
layout: page
---
...
**Source code:** [github.com/jambonz/agent-toolkit](https://github.com/jambonz/agent-toolkit)
**API reference:** [jambonz.github.io/node-sdk](https://jambonz.github.io/node-sdk/)

<Note>
This SDK replaces the older `@jambonz/node-client` (webhook) and `@jambonz/node-client-ws` (WebSocket) packages, which are now [deprecated](/sdks/nodeclient). The new SDK provides a unified package with a consistent API across both transports.
</Note>

<Info>
**Which transport should I use?** The **WebSocket transport** is recommended for most applications. It provides a persistent bidirectional connection that enables TTS streaming, mid-call updates, inject commands, real-time event handling, and voice AI features like the [pipeline](/verbs/verbs/pipeline) and [llm](/verbs/verbs/llm) verbs. The **webhook transport** is simpler but limited — use it for straightforward call routing scenarios (e.g., [dial](/verbs/verbs/dial), basic IVR menus) where you don't need real-time interaction.
</Info>
...
When using the [listen](/verbs/verbs/listen) verb, `makeService.audio()` lets you handle both call control and audio on the same server:
...
`.say()` `.play()` `.gather()` `.dial()` `.llm()` `.pipeline()` `.conference()` `.enqueue()` `.dequeue()` `.hangup()` `.pause()` `.redirect()` `.config()` `.tag()` `.dtmf()` `.listen()` `.transcribe()` `.message()` `.dub()` `.alert()` `.answer()` `.leave()` `.sipDecline()` `.sipRefer()` `.sipRequest()`
...
## LLM and Pipeline Updates

The Session provides methods for interacting with active LLM and pipeline conversations.
...
Send mid-conversation updates to an active [pipeline](/verbs/verbs/pipeline):
...
// Replace tools
session.updatePipeline({ type: 'update_tools', tools: [...] });
...
Send updates to an active [llm](/verbs/verbs/llm) verb:
...
| Method | Description |
|--------|-------------|
| `sendToolOutput(toolCallId, data)` | Send tool/function result back to the LLM or pipeline |
| `updatePipeline(data)` | Send a `pipeline:update` command (update_instructions, inject_context, update_tools, generate_reply) |
...
For Cursor, VS Code, and other editors, see the [setup instructions](https://github.com/jambonz/agent-toolkit#setup) in the repository.

A complementary [Agent Skill](https://github.com/jambonz/skills) provides procedural knowledge about jambonz patterns and best practices:
...
See the [examples directory](https://github.com/jambonz/agent-toolkit/tree/main/examples) for runnable demos:
...
For pipeline verb examples, see the [pipeline examples](https://github.com/jambonz/v10-examples/tree/main/examples/pipeline).
