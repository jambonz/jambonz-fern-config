...
#### Up and running
As mentioned above, the [create-jambonz-app](/sdks/npx) will scaffold you an [express](https://expressjs.com/)-based app that provides a great starting point.
...
If your application just needs to respond to [webhooks](/reference/webhooks/call-hook-post), you can include it like this:
```js
const {WebhookResponse} = require('@jambonz/node-client');
```

If your application also needs to make [REST API calls](/reference/rest-call-control), you can include it like this:
```js
...
The WebhookResponse instance offers methods corresponding to each of the [jambonz verbs](/verbs/verbs/overview/), so for the example above you can see the possible options for the `pause` verb [here](/verbs/verbs/pause/), and for the `say` verb [here](/verbs/verbs/say/).  After adding all of the verbs that you want to execute, in the order that you want them to happen, simply return the `app` object in the 200 OK to the HTTP request.
...
```
Documentation for the REST API can be found [here](/reference/rest-call-control/calls/create-call).
