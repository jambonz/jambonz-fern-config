---
title: Python SDK
subtitle: Build jambonz voice applications with jambonz-python-sdk
layout: page
---
...
<Warning>
The Python SDK is currently **experimental**. APIs may change between releases. Please report issues on [GitHub](https://github.com/jambonz/python-sdk/issues).
</Warning>
...
`.say()` `.play()` `.gather()` `.dial()` `.llm()` `.pipeline()` `.conference()` `.enqueue()` `.dequeue()` `.hangup()` `.pause()` `.redirect()` `.config()` `.tag()` `.dtmf()` `.listen()` `.transcribe()` `.message()` `.dub()` `.alert()` `.answer()` `.leave()` `.sip_decline()` `.sip_refer()` `.sip_request()`
...
# Replace tools
session.update_pipeline({"type": "update_tools", "tools": [...]})
...
| Method | Description |
|--------|-------------|
| `send_tool_output(tool_call_id, data)` | Send tool/function result back to the LLM or pipeline |
| `update_pipeline(data)` | Send a `pipeline:update` command |
