...
| fillerNoise | play audio to the caller while the remote application is processing gather transcriptions. See [Using filler noise](/guides/features/using-filler-noise) for more details.| no |
...
| fillerNoise.startDelaySecs | integer value specifying number of seconds to wait for a response from the remote  [recognizer](/verbs/verbs/recognizer)  Speech recognition options | no |
