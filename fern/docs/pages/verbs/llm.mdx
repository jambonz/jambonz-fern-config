...
<Warning>
The GPT Live Event API reference documents only `delegation.type` on the session object — the `delegation.tools` and `delegation.model` fields above are inferred from the Responses API and are **not yet confirmed against the alpha**. If OpenAI's field names differ, the server accepts the `session.update` and silently drops the tool block, so injected `handoff`, `hangup` and MCP tools would never reach the model. Verify against your alpha access before relying on tools.
</Warning>
