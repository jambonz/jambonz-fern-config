...
<ParamField path="handoff" type="object" required={false}>
  ...
  `transfer_to_human` tool and runs the packaged [transfer](/verbs/transfer) choreography
...
[transfer](/verbs/transfer) choreography to the configured destination.
...
The `handoff` block accepts every [transfer](/verbs/transfer#parameters) option (`mode`,
...
[the agent verb's handoff section](/verbs/agent#transfer-to-human-handoff) for the details,
which apply identically here. When the human leg bridges, the `actionHook` reports the
[transfer outcome](/verbs/transfer#actionhook-properties).
