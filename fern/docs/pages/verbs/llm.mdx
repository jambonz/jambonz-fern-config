...
Set `vendor: 'gptlive'` and supply an OpenAI API key via `auth.apiKey` to use OpenAI's **GPT Live API**, currently a limited-access alpha. jambonz connects to `wss://api.openai.com/v1/live?model=<model>` with a `Bearer` token and the alpha opt-in header `OpenAI-Alpha: quicksilver=v2`, which `/v1/live` requires (the media server sends it; override with the `JAMBONES_GPTLIVE_ALPHA` environment variable if OpenAI bumps the tag).

<Warning>
GPT Live is a **different wire protocol** from the [OpenAI Realtime API](#openai-realtime) — it is not a drop-in replacement for `vendor: 'openai'`. There is no `response_create`, no `response.create`/`response.cancel`, and tool calls arrive through a *delegation* rather than directly.

Access requires enrollment in OpenAI's Early Access Program. A key that is not enrolled completes the websocket handshake and is then refused at the first server event with `{"code":"forbidden","message":"Voice session access denied."}`, which jambonz reports as `completionReason: 'server error'`. Event names and payloads may change while the API is in alpha.
</Warning>
...
There is no `response_create`, so there is no explicit "speak now" trigger like the Realtime API's `response.create`. **The agent opens the conversation on its own as soon as input audio starts flowing** — and because jambonz begins streaming the caller's audio the moment `session.started` lifts the media server's input gate, that happens automatically on every call, typically within a second of the session starting.

Verified against the alpha:

| What you send | Agent speaks first? |
|---|---|
| `instructions` only, no audio at all | **no** — it waits indefinitely |
| Any input audio, even pure silence | **yes** |
| `session.context.append` with no audio | **no** — context alone does not trigger generation |

So the trigger is the audio stream, not the prompt and not a client event. Silence counts: a caller who says nothing still causes the agent to open.

To control *what* it opens with, put it in `session_update.instructions` — that is the only lever, since there is no per-response instruction override:

```js
llmOptions: {
  session_update: {
    instructions: SYSTEM_PROMPT +
      ' Open the conversation yourself: as soon as the call connects, greet the caller ' +
      'and ask how you can help. Do not wait for the caller to speak first.',
...
<Note>
Two consequences of the trigger being audio rather than an explicit event. First, you cannot deterministically sequence "greet, then listen" the way `response.create` allows — the model decides when to start. Second, if the agent is **silent** on a call, the cause is almost always that no audio reached the vendor: check your `eventHook` for `session.started` (without it the media server's input gate never lifts and every caller frame is dropped), and check for an `error` event before it.
</Note>
...
<ParamField path="session_update.delegation.type" type="string" required={false}>
  ...
  `'responses'` — the model runs a Responses API turn. Its `response.*` events stream inline on the same connection, and function calls are answered with `delegation.function_call_output.create`. This variant carries a **required nested object**, `delegation.responses`, whose `model` is mandatory; put your tool definitions in `session_update.delegation.responses.tools` using the flat Responses shape `{type: 'function', name, description, parameters}`.
</ParamField>
...
delegation: {
  type: 'responses',
  responses: {
    model: 'gpt-5.5',          // required — the Responses-side model, NOT the voice model
...
<Note>
`delegation.responses.model` is the model the *delegated* turn runs on, and is separate from the GPT Live voice model in the connection URL (the verb's `model` property). Omitting the nested object, or its `model`, is rejected at session start with `Invalid AVAS session_data … Missing required parameter: 'delegation.responses'` (or `'delegation.responses.model'`); jambonz catches both up front when tools are configured.
</Note>
...
Tool calling requires `delegation.type: 'responses'` — a `client` delegation has no function-calling protocol, so `mcpServers`, `handoff` and `hangup` cannot be declared on it. Configuring any of them without a `responses` delegation is rejected when the verb is executed, rather than starting a call in which the model was never told the tools exist.

With a Responses delegation — declared at `session_update.delegation.responses.tools` — jambonz picks the completed call off `response.output_item.done` (`item.type: 'function_call'`) and routes `{name, args, tool_call_id}` to your `toolHook`. Return the result as a `delegation.function_call_output.create` client event:
...
`session.usage.updated` reports cumulative session usage roughly once a minute, and `session.closed` carries the final totals. jambonz ends the llm verb on `session.closed`, reporting the reason on the `actionHook`.

Errors are treated according to the spec's own distinction. An `error` arriving **before** `session.started` is a startup error: the session never starts and caller audio never flows, so jambonz ends the verb with `completionReason: 'server error'`. An `error` **after** `session.started` is a command error — one rejected client event, such as a stale `delegation_item_id` or an over-long context append — and does **not** end the call; it is forwarded to your `eventHook` so you can recover. A `response.failed` likewise does not end the call: a Responses delegation is a side-channel unit of work, not the voice session, so the model keeps talking.
