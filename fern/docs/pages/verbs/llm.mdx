...
There is no `response_create`, so nothing forces a first turn — and putting the greeting in `instructions` alone does **not** work. Measured against the alpha:

| What you do | Agent opens the call |
|---|---|
| greeting in `session_update.instructions` only | **0 / 5** |
| `session.context.append` after `session.started` | **5 / 5** |

When the model stays quiet it does **not** simply send nothing: it streams `output_audio.delta` frames of **digital silence**. The caller hears dead air while the media server correctly reports playing audio, so never treat playback events as evidence the agent spoke.

Per OpenAI's prompting guide for the alpha, request the greeting with a `session.context.append` that supplies the **intended wording** and says **when** to speak:
...
The same pattern works for any required spoken message — a disclosure, a transfer notice, a closing.

<Warning>
A context append **guides** the model; it is not a playback command. OpenAI's guidance is explicit that the model may stay silent, paraphrase, or be interrupted — and we measured paraphrasing on every attempt when the wording was not supplied. **When exact wording is a legal or brand requirement, do not rely on the model: speak it with a [`say`](/verbs/verbs/say) verb before the `llm` verb** so jambonz TTS plays it deterministically.
</Warning>

<Note>
`session.context.append` is currently reachable only over **WebSocket** (`session.updateLlm(...)`). The REST `updateCall` API does not accept `llm_update`, so a webhook-transport application cannot send it — another reason s2s applications should use WebSocket transport.
</Note>
