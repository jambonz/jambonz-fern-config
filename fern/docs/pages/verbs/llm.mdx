...
<ParamField path="greeting" type="string" required={false}>
  Optional proactive greeting. When set, jambonz sends this text to Gemini immediately after setup so the agent speaks first without waiting for the caller to speak. The value is an **instruction to the model**, not the literal words — for example `"Greet the caller warmly"` rather than `"Hello, how can I help?"`.
