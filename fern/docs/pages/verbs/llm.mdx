...
<Warning title="Alpha API, and not a drop-in for vendor: 'openai'">
GPT Live requires enrollment in OpenAI's Early Access Program — an ordinary OpenAI key is rejected, and jambonz will end the verb with `completionReason: 'server error'`.

It is also a **different protocol** from the [OpenAI Realtime API](#openai-realtime), not a newer model for it. If you are moving an app over from `vendor: 'openai'`, read [Migrating from the Realtime API](#migrating-from-the-realtime-api) — the payloads are not interchangeable. Expect event names and fields to change while the API is in alpha.
</Warning>

Here is a complete working session:
...
<ParamField path="model" type="string" required={false}>
  GPT Live model name; defaults to `gpt-live-1-boulder-alpha`. Set it here on the verb, **not** inside `session_update` — jambonz rejects the verb if you put it in both places.
</ParamField>
...
`llmOptions.session_update` is not optional here. GPT Live will not accept the caller's audio until it has your startup configuration, so jambonz sends `session_update` first and holds the caller's audio until the server confirms with `session.started`.

There is no `response_create`. Once the session starts, the model drives the conversation itself.

### Making the agent speak first

Because there is no `response_create`, nothing tells the model to take the first turn — and **putting the greeting in `instructions` is not enough**. In practice the model waits for the caller, who hears silence and hangs up.
...
<Warning title="This requests a greeting, it does not guarantee one">
A context append **guides** the model. Per OpenAI, it may paraphrase your wording, or occasionally stay silent. Omit the wording and it will paraphrase every time.
...
<Note>
`session.updateLlm(...)` requires **WebSocket** transport. The REST `updateCall` API cannot send GPT Live client events, so a webhook-only application has no way to request a greeting.
</Note>

### Delegation: how the model asks for work

This is the concept that makes GPT Live different from every other s2s vendor. Anything the model needs from outside the spoken conversation — a fact it does not know, a function it wants run — arrives as a **delegation**. You pick which kind you want with `session_update.delegation.type`, and the choice determines whether you can use tools at all.
...
<ParamField path="session_update.delegation.type" type="string" required={false}>
  ...
  `'responses'` — the model runs an OpenAI Responses turn that **can call your functions**. Required if you want tools, `mcpServers`, `handoff` or `hangup`.
</ParamField>

#### `type: 'responses'` — for tools
...
<Warning>
Tools go in `delegation.responses.tools`, and both the nested `responses` object and its `model` are mandatory. jambonz validates this when the verb runs and fails fast with a clear message, rather than starting a call in which the model was never told your tools exist.
</Warning>

#### `type: 'client'` — for supplying context
...
`output` must be a string. Send one of these per call if the model asked for several, and do not send anything else afterwards — unlike the Realtime API there is no follow-on `response.create`, because the server resumes on its own.
...
Subscribe with `events` and filter on `eventHook` as usual. The two most useful:

| Event | What it gives you |
|---|---|
| `turn.done` | A completed utterance with `turn.role` and `turn.transcript` — the readable "who said what" |
| `session.usage.updated` | Cumulative token usage, roughly once a minute; final totals arrive on `session.closed` |

If you want finer granularity, `input_transcript.added` and `output_transcript.added` stream partial fragments as they are recognized. Be aware that fragment boundaries follow speech cadence rather than complete thoughts, so a single sentence may arrive in several pieces — prefer `turn.done` unless you specifically want live partials.

Barge-in is automatic: jambonz flushes queued agent audio as soon as the caller starts talking.

### Updating the session mid-call

`session.updateLlm(...)` accepts these five events, and nothing else:
...
| `connection failure` | jambonz could not reach GPT Live — usually a key without alpha access |
| `disconnect from remote end` | OpenAI dropped the connection |
| `server error` | GPT Live rejected the session at startup |

Not every problem ends the call. Once the session is running, a rejected client event — a stale `delegation_item_id`, an over-long context append — or a failed delegation is reported to your `eventHook` and the conversation continues. Handle those events if you want to recover; ignore them and the model simply carries on without whatever you were trying to give it.

<Note>
`cancelOnBargeIn` and `cancelOnResponseTimeout` have no effect with this vendor, because GPT Live has nothing to cancel. `responseTimeoutMs` does work: if a delegation stalls you get a `response.timeout` event and can recover with `session.updateLlm(...)`.
</Note>
...
| `session_update.turn_detection` | Not supported — endpointing is handled for you |
...
| `input_audio_buffer.speech_started` for barge-in | Handled internally; not surfaced |

### Troubleshooting

| Symptom | Cause |
|---|---|
| The agent never speaks and the caller hears silence | No `session.context.append` on `session.started` — see [Making the agent speak first](#making-the-agent-speak-first) |
| Verb ends immediately with `server error` | The API key is not enrolled in the Early Access Program |
...
| The verb is rejected before the call connects | `model` set in both the verb and `session_update`, or `handoff`/`hangup`/`mcpServers` configured without a `responses` delegation |
