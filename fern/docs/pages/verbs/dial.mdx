...
<ParamField path="anchorMedia" type="boolean" required={false}>
  if true, jambonz will not release the media from feature server for the bridged call. [See here](/guides/features/controlling-the-media-path-during-a-call) for details.  
  Default: false.
</ParamField>
