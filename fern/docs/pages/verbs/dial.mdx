...
<ParamField path="srtpEncryption" type="string" required={false}>
  ...
  Set to `sdes` to negotiate SRTP via SDES crypto attributes in the SDP offer (the `RTP/SAVP` profile with an `a=crypto` line) — this is what SIP endpoints such as LiveKit expect.  
