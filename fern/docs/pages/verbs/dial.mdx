...
| ------------- |-------------| -----|
| actionHook | webhook to invoke when the call ends. The webhook will include [properties](#actionhook-properties) describing the outcome of the call attempt.| no |
...
| anchorMedia | if true, jambonz will not release the media from feature server for the bridged call. [See here](/guides/features/controlling-the-media-path-during-a-call) for details. Default: false | no |
...
| dub | a nested [dub](/verbs/verbs/dub) verb to add additional audio tracks into the outbound call. | no |
| exitMediaPath | if true, jambonz will attempt to re-invite itself completely out of the media path for the call; [see below](#exitmediapath) for details. Defaults to false| no | 
| headers | an object containing arbitrary sip headers to apply to the outbound call attempt(s) | no |
| listen | a nested [listen](/verbs/verbs/listen) action, which will cause audio from the call to be streamed to a remote server over a websocket connection | no |
...
| transcribe | a nested [transcribe](/verbs/verbs/transcribe) action, which will cause the call to be transcribed | no 
...
For more details on controlling the media path during a call, please see [Controlling media path during call](/guides/features/controlling-the-media-path-during-a-call).
