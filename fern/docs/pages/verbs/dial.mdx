...
<ParamField path="codecs" type="array" required={false}>
  Codec names, in preference order, to offer on the outbound INVITE, e.g. `["G722", "PCMU"]`. Names are case-insensitive.  
  When omitted, jambonz offers its default codecs with the inbound leg's codec placed first, so that both legs of the bridged call use the same codec and media can be released from the media server.  
...
<ParamField path="forwardPAI" type="boolean" required={false}>
  Controls if the `P-Asserted-Identity` & `Privacy` headers from the inbound call are forward to the outbound leg.
  Defaults to true.
</ParamField>
...
<ParamField path="srtpEncryption" type="string" required={false}>
  Request encrypted media (SRTP) on an outbound call to a `sip` [target](#target-types).  
...
By default jambonz sends media to a dialed `sip` target as unencrypted RTP. Set `srtpEncryption` to `sdes` to request encrypted media (SRTP) on the outbound leg — jambonz will offer the `RTP/SAVP` profile with an `a=crypto` SDES key in the SDP.

This is independent of signaling encryption: the `sips:` scheme (or `;transport=tls`) on the `sipUri` is what makes the SIP signaling go over TLS, while `srtpEncryption` is what encrypts the media. To secure both — as required by endpoints such as [LiveKit](https://docs.livekit.io/sip/) — use a `sips:` uri together with `srtpEncryption`:
