---
title: Leave
subtitle: Creates a unidirectional or bidirectional audio stream to a remote websocket server.
hide-toc: true
---
...
The listen verb can also be nested in a [dial](/verbs/verbs/dial) or [config](/verbs/verbs/config) verb, which allows the audio for a call between two parties to be sent to a remote websocket server.
...
Note that the remote side can optionally send messages and audio back over the websocket connection, as described below in [Bidirectional Audio](#streaming-bi-directional-audio).
...
| transcribe | a nested [transcribe](/verbs/verbs/transcribe) verb | no |
