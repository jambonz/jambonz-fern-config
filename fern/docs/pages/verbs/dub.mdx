---
title: Dub
subtitle: Add one or more additional audio tracks into the conversation
---
...
<ParamField path="gain" type="string" required={false}>
...
<ParamField path="loop" type="boolean" required={false}>
  If true, loop the mp3.
</ParamField>

<ParamField path="play" type="string" required={false}>
  An http(s) URL to an mp3 file to play into the track.
</ParamField>

<ParamField path="say" type="string" required={false}>
  Text to convert to audio and play into the track.
</ParamField>

<ParamField path="track" type="string" required={true}>
  Label for the track.
</ParamField>
...
See [Using dub tracks](/guides/features/dub-tracks) for more information.
