...
| fillerNoise | play audio to the caller while the remote application is processing gather transcriptions. This is a session-wide setting that may be overridden at the [gather verb](/verbs/verbs/gather) level. See [Using filler noise](/guides/features/using-filler-noise) for more details.| no |
