...
<ParamField path="target" type="array" required={true}>
  One or more call [destinations](/verbs/verbs/dial#target-types) (same structure as the `dial`
...
<ParamField path="amd" type="object" required={false}>
  ...
  destination leg (same `amd` structure as the `dial` verb). Off by default; a `machine`
