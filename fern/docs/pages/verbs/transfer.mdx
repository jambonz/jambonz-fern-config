...
<ParamField path="target" type="array" required={true}>
  One or more call [destinations](/verbs/dial#target-types) (same shape as the `dial`
...
<ParamField path="brief.synthesizer" type="object" required={false}>
  Optional voice/vendor for the brief (a [synthesizer](/verbs/synthesizer) object).
...
<ParamField path="amd" type="object" required={false}>
  `warm` only. Optional [answering-machine detection](/verbs/dial) run on the
...
[agent](/verbs/agent#transfer-to-human-handoff) or [llm](/verbs/llm#transfer-to-human-handoff)
