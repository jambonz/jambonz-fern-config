...
The agent verb wires together three components — STT, LLM, and TTS — with integrated turn detection to create a complete voice AI agent. Unlike the [llm](/verbs/verbs/llm) verb (which connects to speech-to-speech APIs like OpenAI Realtime), the agent verb lets you mix and match separate vendors for each component.
...
<Note>
For a comprehensive guide on building voice AI agents with the agent verb, see [Voice Agents](/guides/features/voice-agents).
</Note>
...
<ParamField path="eventHook" type="string" required={false}>
  A webhook invoked for agent events. Receives event types: `user_transcript`, `agent_response`, `user_interruption`, `turn_end`, and `history_summarized`. See [eventHook Events](#eventhook-events) below.
</ParamField>
...
<ParamField path="handoff" type="object" required={false}>
  ...
  [transfer](/verbs/verbs/transfer) choreography when the model calls it. See
  [Transfer-to-human handoff](#transfer-to-human-handoff).
</ParamField>

<ParamField path="hangup" type="object" required={false}>
  Enable the built-in hangup tool. When present, the runtime injects a `hangup` tool into the LLM's toolset; when the LLM calls it, the call is ended. See [Built-in Hangup Tool](#built-in-hangup-tool) below.
</ParamField>
...
<ParamField path="llm" type="object" required={true}>
  LLM configuration. This is the only required property. See [Bring Your Own LLM](/guides/features/bring-your-own-llm) for the full list of supported vendors and per-vendor setup.
</ParamField>

<ParamField path="llm.vendor" type="string" required={true}>
  LLM vendor id. One of: `anthropic`, `azure-openai`, `baseten`, `bedrock`, `deepseek`, `google`, `groq`, `huggingface`, `moonshot`, `openai`, `vertex-gemini`, `vertex-openai`, `xai`, `zai`. See [Bring Your Own LLM](/guides/features/bring-your-own-llm) for setup of each.
</ParamField>
...
<ParamField path="llm.label" type="string" required={false}>
...
<ParamField path="llm.auth" type="object" required={false}>
  Inline authentication credentials. Format depends on the vendor — see the per-vendor pages under [Bring Your Own LLM](/guides/features/bring-your-own-llm). If not provided, jambonz looks up credentials by `vendor` (and `label`, if the rare multi-credential case applies) from the LLM Services configured in the portal.
</ParamField>
...
<ParamField path="noResponseTimeout" type="number" required={false} default="12">
...
When the caller asks for a human and the model calls the tool, jambonz runs the packaged
...
<ParamField path="handoff.brief" type="string or object" required={false} default="auto">
  Controls the spoken summary delivered to the human on a warm transfer. `'auto'` lets the
...
<ParamField path="handoff.briefSynthesizer" type="object" required={false}>
  Optional voice/vendor ([synthesizer](/verbs/verbs/synthesizer) object) for the spoken brief.
  Defaults to the session synthesizer.
</ParamField>

<ParamField path="handoff.toolName" type="string" required={false} default="transfer_to_human">
  Override the injected tool name.
</ParamField>

<ParamField path="handoff.toolDescription" type="string" required={false}>
...
[transfer actionHook properties](/verbs/verbs/transfer#actionhook-properties)).

## Built-in Hangup Tool

Set `hangup` to let the LLM end the call on its own. The runtime injects a `hangup` tool into the LLM's toolset; you do not define it in `llm.llmOptions.tools` and you do not handle it in your `toolHook` — the runtime intercepts the call, hangs up, and ends the agent verb.
...
<Note>
When the LLM calls the hangup tool, the call is released immediately. The `actionHook` still fires (with `completion_reason: "hangup"`), but any follow-on verbs it returns are discarded because the call is already being torn down.
</Note>
...
| Azure OpenAI | `azure-openai` | _your deployment name_ |
...
<Tip>
See [Bring Your Own LLM](/guides/features/bring-your-own-llm) for per-vendor credential setup, model recommendations, and known issues.
</Tip>

## Example Applications

See the [agent examples](https://github.com/jambonz/v10-examples/tree/main/examples/agent) for runnable demos covering basic usage, tool calling, MCP servers, CRM injection, persona switching, and more.
