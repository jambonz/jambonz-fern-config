...
<ParamField path="llm.vendor" type="string" required={true}>
  LLM vendor id. One of: `openai`, `anthropic`, `google`, `vertex-gemini`, `vertex-openai`, `bedrock`, `deepseek`, `azure-openai`, `groq`, `huggingface`. See [Bring Your Own LLM](/guides/features/bring-your-own-llm) for setup of each.
</ParamField>
...
<ParamField path="llm.label" type="string" required={false}>
  Optional credential label. Use to disambiguate when the account has multiple LLM credentials for the same vendor.
</ParamField>

<ParamField path="llm.auth" type="object" required={false}>
  Inline authentication credentials. Format depends on the vendor — see the per-vendor pages under [Bring Your Own LLM](/guides/features/bring-your-own-llm). If not provided, jambonz looks up credentials by `(vendor, label)` from the LLM Services configured in the portal.
</ParamField>
...
| HuggingFace Inference Providers | `huggingface` | `meta-llama/Llama-3.3-70B-Instruct`, `…:fastest` |
