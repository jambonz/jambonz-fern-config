...
<ParamField path="handoff" type="object" required={false}>
  ...
  [transfer](/verbs/transfer) choreography when the model calls it. See
...
<ParamField path="llm.vendor" type="string" required={true}>
  LLM vendor id. One of: `anthropic`, `azure-openai`, `baseten`, `bedrock`, `deepseek`, `google`, `groq`, `huggingface`, `openai`, `vertex-gemini`, `vertex-openai`. See [Bring Your Own LLM](/guides/features/bring-your-own-llm) for setup of each.
</ParamField>
...
<ParamField path="handoff.briefSynthesizer" type="object" required={false}>
  Optional voice/vendor ([synthesizer](/verbs/synthesizer) object) for the spoken brief.
...
[transfer actionHook properties](/verbs/transfer#actionhook-properties)).
