The `recognizer` property is used in multiple verbs ([gather](/verbs/verbs/gather), [transcribe](/verbs/verbs/transcribe), [dial](/verbs/verbs/dial)). It selects and configures the speech recognizer.  

It is an object containing the following properties:

| option        | description | required  |
| ------------- |-------------| -----|
| vendor | Speech vendor to use (see list below, along with any others you add via the [custom speech API](/guides/speech-providers/custom-speech-api)) | no |
...
| asrTimeout|timeout value for [continuous ASR feature](/guides/features/continuous-asr)| no |
| asrDtmfTerminationDigit|DMTF key that terminates [continuous ASR feature](/guides/features/continuous-asr)| no |
...
Note: Microsoft supports [on-prem and private link options](/guides/features/azure-private-link) for deploying the speech service in addition to the hosted Microsoft service.
