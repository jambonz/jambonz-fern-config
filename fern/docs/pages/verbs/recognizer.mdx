...
<AccordionGroup>
  ...
  <Accordion title="googleOptions">
    Google serves three speech APIs from one set of credentials: Cloud Speech-to-Text v1, Cloud Speech-to-Text v2, and the Gemini Live transcription models. Which one runs is chosen by the recognizer `model`: a name beginning with `gemini-` selects the Gemini Live API, anything else a Cloud Speech recognizer. Leave `model` unset and jambonz picks a Cloud Speech model appropriate to the language, as it always has. A default model can also be stored on the Google speech credential in the jambonz portal, which applies to every call on that account unless the request overrides it.
    ...
    Google serves the Gemini models through two interfaces that take different credentials and reject each other's: **studio**, the preview Gemini Developer API, authenticates with an AI Studio api key stored on the speech credential; **vertex**, the GA Vertex AI interface, authenticates as the credential's service account and additionally needs billing and `roles/aiplatform.user`, taking the project from the service account and the region from `parentPath`. jambonz picks the interface from the credential — an api key selects studio, a credential with only a service account selects vertex — and `geminiPlatform` overrides that.

    <ParamField path="geminiPlatform" type="string" required={false}>
      Force the interface: `studio` or `vertex`. Inferred from the credential when unset.
    </ParamField>

    <ParamField path="apiMode" type="string" required={false}>
      Which API to use: `v1`, `v2` or `gemini`. Normally inferred from the model name; set this only to override that inference.
    </ParamField>

    <ParamField path="mode" type="string" required={false}>
      Gemini transcription style (`gemini` only): `VERBATIM` (default) transcribes speech as spoken, `SMART` cleans up disfluencies and applies formatting.
    </ParamField>

    <ParamField path="customVocabulary" type="array" required={false}>
      Up to 1000 terms, acronyms, brand names or proper nouns to bias recognition towards (`gemini` only). Example: `["jambonz", "drachtio"]`. When omitted, the recognizer's `hints` are used instead.
    </ParamField>

    <ParamField path="serviceVersion" type="string" required={false}>
      Cloud Speech-to-Text API version, `v1` (default) or `v2`. Ignored when `apiMode` is `gemini`.
    </ParamField>

    <ParamField path="recognizerId" type="string" required={false}>
      ID of a Google Speech recognizer resource (v2 only).
    </ParamField>

    <ParamField path="parentPath" type="string" required={false}>
    ...
    <ParamField path="speechStartTimeoutMs" type="number" required={false}>
      Milliseconds to wait for speech to start (v2 only).
    </ParamField>

    <ParamField path="speechEndTimeoutMs" type="number" required={false}>
      Milliseconds of silence used to detect the end of speech (v2 only).
    </ParamField>

    <ParamField path="enableVoiceActivityEvents" type="boolean" required={false}>
      Enable Google's voice activity detection events (v2 only).
    </ParamField>

    <ParamField path="transcriptNormalization" type="array" required={false}>
