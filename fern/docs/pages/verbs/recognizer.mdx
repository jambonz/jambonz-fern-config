...
<AccordionGroup>
  ...
  <Accordion title="houndifyOptions">
    ...
    <ParamField path="requestInfo" type="object" required={false}>
      Houndify request info passed to the Polaris WebSocket session.
    </ParamField>

    <ParamField path="requestInfo.segmentation" type="object" required={false}>
      Controls how Houndify segments the input audio stream into utterances.
    </ParamField>

    <ParamField path="requestInfo.segmentation.mode" type="string" required={false} default="semantic">
      Segmentation strategy. One of:
      - `"semantic"` (default) or `"sentence"` — Houndify performs server-side segmentation
        and emits a `FinalSegmentTranscript` for each stabilized piece. jambonz buffers each
        segment and concatenates them into the final transcript when the session ends.
      - `"none"` — single-query, client-driven endpointing. jambonz uses `eoq` / `vad`
        thresholds (see `maxSilenceSeconds`, `vadTimeout`) to detect end-of-turn and only
        the last `PartialTranscript` (or the `FinalTranscript`, if returned) is used as the
        result.
    </ParamField>
